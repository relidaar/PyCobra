...
class TestLexerDataTypes(TestCase):
    ...
    def test_check_float(self):
        ...
        positive_float_without_first_digit = '.5'
        positive_float_without_first_digit_result = list(lexer.lex(positive_float_without_first_digit))[0]
        self.assertEqual(positive_float_without_first_digit_result.gettokentype(), 'FLOAT')

        negative_float = '-5.5'
        negative_float_result = list(lexer.lex(negative_float))[0]
        self.assertEqual(negative_float_result.gettokentype(), 'FLOAT')

        negative_float_without_first_digit = '-.5'
        negative_float_without_first_digit_result = list(lexer.lex(negative_float_without_first_digit))[0]
        self.assertEqual(negative_float_without_first_digit_result.gettokentype(), 'FLOAT')
